import { describe, it, expect, vi, beforeEach, afterEach } from 'vitest';

vi.mock('../../../services/api/categoria.service', () => ({
    categoriaService: { getAll: vi.fn().mockResolvedValue([]) },
}));

import React, { act } from 'react';
import { createRoot, type Root } from 'react-dom/client';

(globalThis as unknown as { IS_REACT_ACT_ENVIRONMENT: boolean }).IS_REACT_ACT_ENVIRONMENT = true;

import { FormReceita } from './FormReceita';

let container: HTMLDivElement;
let root: Root;

/** O campo não tem `id` nem `htmlFor`; o placeholder é o único gancho estável hoje. */
const campoValor = (): HTMLInputElement => {
    const el = container.querySelector<HTMLInputElement>('input[placeholder="0,00"]');
    if (!el) throw new Error('Campo "Valor" não encontrado');
    return el;
};

const digitar = (input: HTMLInputElement, texto: string) => {
    const setter = Object.getOwnPropertyDescriptor(
        window.HTMLInputElement.prototype,
        'value'
    )!.set!;
    act(() => {
        setter.call(input, texto);
        input.dispatchEvent(new Event('input', { bubbles: true }));
    });
};

beforeEach(() => {
    container = document.createElement('div');
    document.body.appendChild(container);
    root = createRoot(container);
});

afterEach(() => {
    act(() => root.unmount());
    container.remove();
});

const montar = (onSave = vi.fn().mockResolvedValue(true)) => {
    act(() => {
        root.render(
            React.createElement(FormReceita, { postoId: 1, onSave, onCancel: () => {} })
        );
    });
    return onSave;
};

describe('FormReceita — campo de valor em padrão monetário', () => {
    it('abre com o campo vazio, deixando o placeholder aparecer', () => {
        montar();
        expect(campoValor().value).toBe('');
        expect(campoValor().required).toBe(true);
    });

    it('aceita digitação em formato brasileiro e salva o valor em reais', async () => {
        const onSave = montar();
        digitar(campoValor(), '1.250,90');
        expect(campoValor().value).toBe('1.250,90');

        await act(async () => {
            container.querySelector('form')!.dispatchEvent(
                new Event('submit', { bubbles: true, cancelable: true })
            );
        });

        expect(onSave).toHaveBeenCalledWith(expect.objectContaining({ valor: 1250.9 }));
    });
});
