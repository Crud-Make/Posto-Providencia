...
/**
 * `POST` na API Laravel com a resposta validada por schema. 204 chega como `null` ao schema.
 *
...
export function postarNaApi<T>(caminho: string, corpo: unknown, token: string | null, schema: z.ZodType<T>): ResultAsync<T, ErroDeApi> {
  ...
  return ResultAsync.fromPromise(
    fetch(`${base}${caminho}`, { method: 'POST', headers: cabecalhos(token), body: JSON.stringify(corpo ?? {}) }),
    erroDeRede,
  )
    .andThen((resposta) => {
