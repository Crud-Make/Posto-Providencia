...
- Web: `apps/web/src/contexts/PostoContext.tsx` (default 1, localStorage, fallback hardcoded
...
- PWAs: `const POSTO_ID = 1` em `apps/pwa-frentista/src/App.tsx`, `TanquesScreen.tsx`,
  `apps/pwa-dono/src/screens/EncerranteScreen.tsx`, `EnviosScreen.tsx`; `App.tsx` também crava
...
legível mais próxima é `packages/types/src/database.types.ts` (defasado: não tem HistoricoTanque,
...
`apps/web/src/components/fechamento-mensal/index.tsx`. `combustivel.service.ts` tem
...
rg -n 'onClick' apps/web/src/components/configuracoes/components/GestaoBicos.tsx  # vazio = só lista
rg -n 'setPostoAtivo(ById)?\(' apps/web/src | grep -v PostoContext      # quem troca posto na UI
