...
- `4a908c1` (09/01/2026, *"refactor(api): modularizacao do api.ts em services
  ...
  `frontend/apps/web/src/services/api/compra.service.ts` (+87 linhas). Corpo vazio.
...
- `97fee45` (26/07/2026, *"feat(utils): módulo canônico de lucro + golden master
  vs planilha real"*) cria `frontend/packages/utils/src/lucro.ts`. O corpo justifica o
...
`frontend/packages/utils` em julho/agosto e a escrita simplesmente ficou onde estava.

**Estado em 28/08/2026 — 3 cópias eram, 2 são:**
- `frontend/apps/web/src/services/api/compra.service.ts:117` (`Estoque.custo_medio`) — VIVA
- `frontend/apps/web/src/services/stockService.ts:88` (`preco_custo`) — VIVA
...
`frontend/packages/utils/src/estoque-encadeamento.golden.spec.ts`. Ele **replica** a
ponderada porque `frontend/packages/*` não importa de `frontend/apps/*` (§2). Mexer no custo dentro
de `frontend/apps/web` **não** dispara esse golden.
