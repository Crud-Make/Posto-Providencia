...
# ANDAMENTO — trilha estrutural, 28/08/2026

Branch `chore/saneamento-estrutural`, worktree `../Posto-Providencia-estrutural`.
**Cinco portões verdes ao fim:** `type-check` limpo (agora **com os goldens dentro**) · `lint` limpo
· golden **3233 pass / 0 fail** · vitest **362 pass / 41 arquivos** · `build` ✅.

| item | estado | o que houve |
|---|---|---|
| **1.6** goldens no compilador | ✅ | 31 erros eram tipos do Bun ausentes; `@types/bun` instalado (aprovado). Sobrou **1 erro real**: `encerrante-mensal.golden.spec.ts:188` tipava `db.query` com 2 parâmetros para um SQL de 3 placeholders, e dois `as` mascaravam. Sem efeito em runtime — o `bun:sqlite` ligava os três —, mas era uma mentira de tipo num golden que nunca via o compilador. Corrigido para `[number, number, number]`, casts removidos |
| **1.2** 4 `enum` mortos | ✅ | `packages/types/src/database/enums.ts` apagado + linha do barril. Confirmado antes: os `StatusFechamento`/`FormaPagamento` que o grep acha vêm de `apps/web/src/types/database`, árvore de tipos do **próprio app** — homônimos, não os do pacote |
| **1.3** o `any` de produção | ✅ | A causa era `CellProps.field: string` **alargando** a união que o componente pai já declarava certa. Nomeada como `CampoDetalhamento` e usada nos dois; `as any` e o `eslint-disable` saíram |
| **1.4** 2 shims órfãos | ✅ | `components/TelaGestaoClientes.tsx` e `TelaConfiguracoes.tsx`. `App.tsx` importa os reais dentro de `clientes/` e `configuracoes/` |
| **1.5** 5 aliases fantasma | ✅ | `@app`, `@pages`, `@widgets`, `@features`, `@entities` entraram no `vite.config.ts`, **antes** do `@` genérico. Provado com import real por `@widgets/` num arquivo e `build` verde; a prova foi revertida |
| **bônus** `env` morto | ✅ | `vite.config.ts` declarava `loadEnv` e nunca usava. Achado pelo `noUnusedLocals` |
| **1.7** §4 do `CLAUDE.md` | ⏸ | **Aguarda decisão do dono** — é mudança de regra de dinheiro, não de código. Redação proposta no relatório |
| **5.3** `strict` | ⏸ **medido, adiado** | Custo real: `strict` **47** · `noImplicitAny` 23 · `strictNullChecks` 29 · `verbatimModuleSyntax` **285** · `noUnusedLocals` 2. Adiado **não por dificuldade**: os 47 caem em `stockService.ts`, `salesAnalysis.service.ts`, `fechamento-mensal/index.tsx`, `useAnaliseCustos.ts` e `financeiro/**` — arquivos da trilha do Fable. Ligar agora era colisão garantida |

**Nada foi commitado** — as mudanças estão na worktree, prontas para revisão.
**Dependência nova:** `@types/bun@1.4.0` em `devDependencies` (§0.2, aprovada em 28/08).
