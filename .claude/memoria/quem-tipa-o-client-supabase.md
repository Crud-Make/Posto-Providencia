...
|---|---|---|
| `frontend/packages/types/src/database.types.ts` | Supabase CLI | **zero importadores**, 1611 linhas mortas |
| `frontend/apps/web/src/types/database/generated.ts` | Supabase CLI | quase em dia — **e não tipa nada** |
| `frontend/apps/web/src/types/database/schema.ts` + `tables/*.ts` | **mão** | **é o `Database` do `createClient`** |
| `frontend/packages/types/src/database/tables/operacoes.ts` | mão | só `Frentista`, `Produto`, `Escala` |

`frontend/apps/web/src/services/supabase.ts:2` importa `Database` de `../types/database` —
o **escrito à mão**. E `frontend/apps/pwa-frentista/src/lib/supabase.ts` chama `createClient`
...
`frontend/apps/web/src/types/database/tables/*.ts`.
...
- `diferenca` — **não existe no banco**, mas está como `number` obrigatório em
  `frontend/packages/types/src/database/tables/operacoes.ts:96`. Coluna fantasma.
