...
export const aggregatorService = {
   ...
   */
  async fetchSettingsData(postoId?: number) {
   ...
   */
  async fetchClosingData(postoId?: number) {
  ...
  async fetchAttendantsData(postoId?: number) {
  ...
  async fetchInventoryData(postoId?: number) {
   ...
   */
  async fetchProfitabilityData(year: number = new Date().getFullYear(), month: number = new Date().getMonth() + 1, postoId?: number) {
