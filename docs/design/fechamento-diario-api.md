...
| P9 | `useCustoMensal` sai do Supabase direto | sim (Fable/Opus 5.5) | **passos 1–2 FEITOS em 22/09/2026** (`8aef1b8`), estruturais — nenhum número de dinheiro e nenhuma fonte mudam. Passo 1: caracterização com o Supabase mockado, `useCustoMensal.test.ts` (14) e `registro-compras/hooks/useDespesaDoMes.test.ts` (8): janela do mês cortada em hoje (`intervaloDoMes`, `utils/periodo.ts:45-50`), produto sem compra = `null`, custo = Σvalor/Σlitros, rateio = despesa ÷ litros do encerrante, `posto_id` em toda consulta; os defeitos ficam fixados e marcados (despesa somada em float, D5, `.error` ignorado virando 0). Passo 2: nasce `hooks/custo-mensal.ts` (`calculaCustoMensal`, `:95-109`, 11 casos em `custo-mensal.test.ts`); o hook só busca (`useCustoMensal.ts:42-61`) e delega (`:65-70`), mesma assinatura. Canários: janela no mês civil → 2 vermelhos; `temDespesa` com `> 1` → vermelho nos dois testes. test:golden 3564/0 antes e depois. **Passos 3–6 (troca para a API) ESPERAM:** (i) o `GET /dashboard` sair do grupo público (`routes/api.php:50,63`) para `posto.acesso:gerir` — em execução na branch `feat/#103-fecha-dashboard`; (ii) **D3**, decisão do dono 22/09: os litros do rateio são os do ENCERRANTE, e quem muda é a API (`Agregacao/Application/DadosDoPeriodo.php:135-139` soma `litros_vendidos`); (iii) **D1/D2**, decisão do dono 22/09: compra e despesa no mês civil inteiro (o painel corta em hoje, `periodo.ts:45-50`; a API já usa o mês civil, `DadosDoPeriodo.php:73` para compra e `:129-133` para despesa); (iv) golden de paridade painel × API. **Em aberto:** D5 (leituras com `dia: 1` sem `.order()`, `custo-mensal.ts:55`), fatia de fórmula separada; e a decisão CA-7 sobre Compra/Despesa que esta linha já pedia |
