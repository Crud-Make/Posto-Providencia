...
| `frontend/apps/pwa-frentista` | envio do fechamento do turno, tanques, vendas de loja, presença. Em migração para FSD desde 19/09, com alias próprio `@frentista/` (o `@/` do monorepo é o web): só `shared/api` (client `supabase`) e `shared/ui` (`ReloadPrompt`) têm código, cada um com `index.ts`; `pages/`, `widgets/` e `features/` guardam só os canários das travas; `App.tsx`, `screens/`, `components/`, `services/` e `lib/` seguem fora das camadas | `@posto/utils`, `@posto/api-core`, supabase-js | 19 arquivos, 2,8 k |
