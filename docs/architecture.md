...
**Custo do mês do fechamento diário (#103 P9 passos 1–2, 22/09):** segue no Supabase. `useCustoMensal` (`components/fechamento-diario/hooks/useCustoMensal.ts:42-61`) lê `Leitura`, `Compra` e `Despesa` do posto na janela de `intervaloDoMes` (mês corrente cortado em hoje) e passa as linhas a `calculaCustoMensal` (`custo-mensal.ts:95-109`), função pura: custo médio por produto por `custoMedioCompra` (`null` sem compra), litros do mês por `encerranteMensal`, rateio por `despesaOperacionalPorLitro`. Os defeitos conhecidos ficaram presos por teste de caracterização, não corrigidos: a despesa é somada em float (`custo-mensal.ts:102`), toda leitura entra com `dia: 1` sem `.order()` (D5, `custo-mensal.ts:55`) e o `.error` do Supabase vira lista vazia (`useCustoMensal.ts:63-70`). Os passos 3–6 (ler pelo `GET /api/postos/{posto}/dashboard` de `App\Agregacao`) **não existem ainda**: esperam o `/dashboard` sair do grupo público (`backend/routes/api.php:50,63`) para `posto.acesso:gerir`, a API somar os litros do rateio pelo encerrante em vez de `SUM(litros_vendidos)` (`DadosDoPeriodo.php:135-139`), o painel passar compra e despesa para o mês civil, e um golden de paridade.
