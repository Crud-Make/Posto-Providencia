...
> **A assimetria oxlint × eslint foi fechada no `pre-commit` em 19/09/2026:** ele roda a catraca
> do ESLint nos arquivos do índice (conteúdo: `any`, `toISOString`, type-aware, FSD, neverthrow) e o
> `tsc`. Desde 24/09 o `pre-push` roda só o golden (e barra a `main`); o ESLint completo é do CI, que
> é obrigatório no merge.
...
2. ~~**Fechar a assimetria oxlint × eslint**~~ — feito no `pre-commit` em 19/09; desde 24/09 o
   `pre-push` roda só o golden, e o ESLint completo fica com o CI.
